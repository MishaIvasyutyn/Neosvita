		...
		if (type) {
			...
			switch (type) {
				...
				case 2: // фильтр по пропускам
					...
					// if (!value) {
					// 	block.find(".list__filterVal").addClass("error");
					// }
...
function skipsCountWithDayFilter(userDaySkeeps){
	userDaySkeeps.each(function(){
		...
		sumCell.data("sum", sumCell.text());
		sumCell.html($(this).find(".pp").length);

		sumCell = sumCell.siblings(".bpCount");
		sumCell.data("sum", sumCell.text());
		sumCell.html($(this).find(".bp").length);

		sumCell = sumCell.siblings(".sumCount");
		sumCell.data("sum", sumCell.text());
		sumCell.html($(this).find(".pp, .bp").length);
	});
}
